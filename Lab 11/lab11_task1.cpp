...
void HashTable::insert(string key, string value)
{
	...
	//traverse the array size until an open index is found. If found add Node to that index.
	while(true){
		if(nodeArray[index] == nullptr){
			...
			//linear probing means if the space is not open just increase the index by one. make sure the index is not out of range of the size of the array.
...
string HashTable::search(string key)
{
	...
	//if index is null then the key doesn't exist. Continue going down the array (looping up if necessary) to check for the node.
	while(true){
		if(nodeArray[index] == nullptr){
